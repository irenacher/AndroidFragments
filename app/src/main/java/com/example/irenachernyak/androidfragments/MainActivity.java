...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // this will create activity with the proper fragment based on device orientation
        Configuration configInfo = getResources().getConfiguration();
        if(configInfo.orientation == Configuration.ORIENTATION_LANDSCAPE){
            FragmentLandscape fl = new FragmentLandscape();
            fragmentTransaction.replace(android.R.id.content, fl);
        } else if(configInfo.orientation == Configuration.ORIENTATION_PORTRAIT){
            FragmentPortrait fp = new FragmentPortrait();
            fragmentTransaction.replace(android.R.id.content, fp);
        }
